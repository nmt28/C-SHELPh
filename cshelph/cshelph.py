...
def bin_data(dataset, lat_res, height_res):
    ...
    lat_bin_number = round \
        (abs(dataset['latitude'].min() - dataset['latitude'].max() ) /lat_res)
    height_bin_number = round(abs
        (dataset['photon_height'].min() - dataset['photon_height'].max() ) /height_res)
    ...
    # Cut height bins
    height_bins = pd.cut(dataset['photon_height'], height_bin_number, labels = np.round
        (np.linspace(dataset['photon_height'].min(), dataset['photon_height'].max(), num=height_bin_number), decimals = 1))
...
def get_sea_height(binned_data, surface_buffer=-0.5):
    ...
    grouped_data = binned_data_sea.groupby(['lat_bins'], group_keys=True, observed=False)
    ...
    sea_height_1 = np.where \
        ((sea_height > (mean + 2* sd)) | (sea_height < (mean - 2 * sd)), np.nan,
        sea_height).tolist()
...
def get_water_temp(data_path, latitude, longitude):
    ...
    minx, miny, maxx, maxy = np.min(location_df['longitude']), np.min(
            location_df['latitude']), np.max(location_df['longitude']), np.max(
    ...
    collections = Query.keyword(
        'GHRSST Level 4 CMC0.1deg Global Foundation Sea Surface Temperature Analysis').bounding_box(
        minx, miny, maxx, maxy).temporal(date_range, date_range).get(10)

    short_name = collections[0]["umm"]["ShortName"]

    Query = earthaccess.granule_query().short_name(short_name).version(
        "3.0").bounding_box(minx, miny, maxx, maxy).temporal("2020-01-01", "2020-01-01")

    granules = Query.get(10)

    ds_L3 = xr.open_mfdataset(earthaccess.open(granules), combine='nested',
                              concat_dim='time', coords='minimal')

    sea_temp = ds_L3['analysed_sst'].sel(lat=lat_med, lon=lon_med,
                                         method='nearest').load()
...
def refraction_correction(water_temp, water_surface, wavelength, photon_ref_elev,
                          ph_ref_azimuth, photon_z, photon_x, photon_y, ph_conf,
                          satellite_altitude):
    '''
    ...
    return (out_x, out_y, out_z, ph_conf, photon_x, photon_y, photon_z, ph_ref_azimuth,
            photon_ref_elev)  # We are most interested in out_x, out_y, out_z
...
def get_bath_height(binned_data, percentile, WSHeight, height_resolution):
    ...
    binned_data_bath = binned_data[
        (binned_data['photon_height'] < WSHeight - (height_resolution * 2))]
    grouped_data = binned_data_bath.groupby(['lat_bins'], group_keys=True, observed=False)
    data_groups = dict(list(grouped_data))

    # Create a percentile threshold of photon counts in each grid, grouped by both x and y axes.
    count_threshold = np.percentile(
        binned_data.groupby(['lat_bins', 'height_bins'], observed=False).size().reset_index().groupby('lat_bins', observed=False)[[0]].max(), percentile)
    ...
    for k, v in data_groups.items():
        ...
        if new_df.iloc[bath_bin]['latitude'] >= count_threshold:
            ...
            geo_longitude.append(
                v.loc[v['height_bins'] == bath_bin_h, 'cor_longitude'].values)
            geo_latitude.append(
                v.loc[v['height_bins'] == bath_bin_h, 'cor_latitude'].values)

            bath_bin_median = v.loc[
                v['height_bins'] == bath_bin_h, 'cor_photon_height'].median()
    ...
    geo_df = pd.DataFrame(
            {'longitude'    : geo_longitude_list, 'latitude': geo_latitude_list,
             'photon_height': geo_photon_list, 'depth': geo_depth})
...
def produce_figures(binned_data, bath_height, sea_height, y_limit_top, y_limit_bottom,
                    percentile, file, geo_df, RefY, RefZ, laser, epsg_num):
    '''Create figures'''

    # Create bins for latitude
    x_bins = np.linspace(binned_data.latitude.min(), binned_data.latitude.max(),
                         len(sea_height))

    # Create new dataframes for median values
    bath_median_df = pd.DataFrame.from_dict({'x': x_bins, 'y': bath_height},
                                            orient='index')
    ...
    # plot raw points
    plt.scatter(x=binned_data.latitude, y=binned_data.photon_height, marker='o', lw=0,
                s=1, alpha=0.8, c='yellow', label='Raw photon height')
    ...
    plt.scatter(sea_median_df.x, sea_median_df.y, marker='o', c='b', alpha=1, s=0.5,
    ...
    plt.tight_layout()
    plt.savefig(file + '_gt' + str(laser) + '_' + str(percentile) + '_EPSG' + str(
        epsg_num) + '_' + timestr + ".png")
    # plt.show()
    # plt.close()
    ...
    transformer = Transformer.from_crs("EPSG:" + str(epsg_num), "EPSG:4326",
                                       always_xy=True)
    # print(transformer)
    lon_wgs84, lat_wgs84 = transformer.transform(geo_df.longitude.values,
                                                 geo_df.latitude.values)

    geo_df['lon_wgs84'] = lon_wgs84
    geo_df['lat_wgs84'] = lat_wgs84

    geodf = geopandas.GeoDataFrame(geo_df,
                                   geometry=geopandas.points_from_xy(geo_df.lon_wgs84,
                                                                     geo_df.lat_wgs84))

    geodf.set_crs(epsg=4326, inplace=True)

    geodf.to_file(file + '_gt' + str(laser) + '_' + str(percentile) + '_EPSG' + str(
        epsg_num) + '_' + timestr + ".gpkg", driver="GPKG")
